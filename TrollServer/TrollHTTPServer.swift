...
class TrollHTTPServer {
    ...
    private func deviceInfo() -> HTTPResponse {
        ...
        if fm.fileExists(atPath: uuidPath),
           let stored = try? String(contentsOfFile: uuidPath, encoding: .utf8).trimmingCharacters(in: CharacterSet.whitespacesAndNewlines),
           !stored.isEmpty {
            deviceUUID = stored
        } else {
    ...
    // MARK: - Shell 命令执行 & Gestalt 修改

    /// POST /api/exec  → 在手机上执行 shell 命令，返回 stdout/stderr
    private func handleExec(_ req: HTTPRequest) -> HTTPResponse {
        guard let cmd = String(data: req.body, encoding: .utf8), !cmd.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines).isEmpty else {
            return HTTPResponse(statusCode: 400, headers: [:], body: "缺少命令".data(using: .utf8)!)
        }

        let result = runShellCommand(cmd, environment: ["PATH": "/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin"])

        let outStr = result.stdout
        let errStr = result.stderr

        print("[Shell] 退出码=\(result.exitCode)")
