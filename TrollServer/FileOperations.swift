...
class FileOperations {
    ...
    /// 获取文件修改时间
    func fileModificationDate(_ relativePath: String) -> Date? {
        guard let fullPath = try? resolvePath(relativePath) else { return nil }
        guard let attrs = try? fm.attributesOfItem(atPath: fullPath) else { return nil }
        return attrs[.modificationDate] as? Date
    }
    
    /// 复制文件
    func copyFile(from srcPath: String, to dstPath: String) throws {
        let srcFull = try resolvePath(srcPath)
        let dstFull = try resolvePath(dstPath)
    ...
    /// 移动/重命名文件
    func moveFile(from srcPath: String, to dstPath: String) throws {
        let srcFull = try resolvePath(srcPath)
        let dstFull = try resolvePath(dstPath)
        ...
        try fm.moveItem(atPath: srcFull, toPath: dstFull)
    }
    
    /// 获取文件类型（扩展名）
    func fileExtension(_ relativePath: String) -> String {
        let decodedPath = relativePath.removingPercentEncoding ?? relativePath
        return (decodedPath as NSString).pathExtension.lowercased()
    }
    
    /// 检查目录是否为空
    func isEmptyDirectory(_ relativePath: String) -> Bool {
        guard let contents = try? listDirectory(relativePath) else { return true }
    ...
    func availableDiskSpace() -> Int64 {
        do {
            let attrs = try fm.attributesOfFileSystem(forPath: basePath)
            if let free = attrs[.systemFreeSize] as? Int64 {
                return free
            }
        } catch {
            print("[FileOps] 获取磁盘空间失败: \(error)")
        }
        return 0
    }
