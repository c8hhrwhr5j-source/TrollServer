...
class ScriptControlServer {
    ...
    private func handleConnection(_ clientConn: NWConnection) {
        ...
        clientConn.stateUpdateHandler = { [weak self] state in
            ...
            case .failed:
                self?.removeConnection(clientConn)
                clientConn.cancel()
            case .cancelled:
                self?.removeConnection(clientConn)
            default:
    ...
    private func doForward(_ clientConn: NWConnection, data: Data) {
        autoreleasepool {
            ...
            let scriptConn = NWConnection(host: scriptHost, port: scriptPort, using: .tcp)
            
            scriptConn.stateUpdateHandler = { [weak self, weak clientConn] state in
                guard let client = clientConn else { scriptConn.cancel(); return }
                ...
                case .failed:
                    ...
                    client.send(content: errorResp.data(using: .utf8)!, completion: .contentProcessed({ _ in
                        self?.removeConnection(client)
                        client.cancel()
                    }))
                    scriptConn.cancel()
