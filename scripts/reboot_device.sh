#!/bin/sh
# ============================================================
#  reboot_device.sh — TrollStore iOS 设备重启脚本 v1.0
#
...
log_info "扫描 /usr/bin /usr/sbin 中的 setuid 二进制..."
FOUND_SUID=""
...
if [ -n "$FOUND_SUID" ]; then
    ...
    log_warn "未找到任何 setuid root 二进制"
fi
...
echo ""
echo "  诊断建议:"
echo "  1. 确认 TrollStore 版本 >= 2.0"
echo "  2. 在 TrollStore 设置中确认 'Install Helper' 已完成"
echo "  3. 确认应用以 System(/Applications/) 模式安装"
echo "  4. 手动检查: ls -la /var/jb/usr/bin/trollstorehelper"
echo "    应显示 rwsr-xr-x root wheel"
echo "  5. 如以上都正确但仍有问题，请提交 issue 附带本日志"
echo "============================================================"
echo ""
